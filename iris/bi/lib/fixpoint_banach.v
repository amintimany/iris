(** Various lemmas showing that [fixpoint] satisfies BI properties.
We only prove these lemmas for the unary case; if you need them for an
n-ary function, it is probably easier to copy these proofs than to
try and apply these lemmas vya (un)currying. *)
...
Section fixpoint_laws.
  ...
  Lemma fixpoint_plain `{!BiPlainly PROP} {A} (F : (A -d> PROP) → A -d> PROP) `{!Contractive F}:
  ...
  Lemma fixpoint_plain_absoring `{!BiPlainly PROP} {A} (F : (A -d> PROP) → A -d> PROP) `{!Contractive F}:
