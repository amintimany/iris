...
Section list.
  ...
  Lemma big_sepL_always `{AffineBI PROP} Φ l :
  ...
  Proof.
    ...
    rewrite bare_always_sep_dup -assoc [(⬕ _ ∗ _)%I]comm -!assoc assoc.
    apply sep_mono.
    - rewrite (forall_elim 0) (forall_elim x) pure_True // True_impl.
      by rewrite bare_always_elim wand_elim_l.
    - rewrite comm -(IH (Φ ∘ S) (Ψ ∘ S)) /=.
      apply sep_mono_l, bare_mono, always_mono.
  ...
  Global Instance big_sepL_persistent1 Φ l :
    (∀ k x, PersistentP (Φ k x)) →
    l ≠ [] →
    PersistentP ([∗ list] k↦x ∈ l, Φ k x).
  Proof.
    intros. rewrite /PersistentP (big_opL_commute1 bi_always (R:=(≡))) //.
    apply big_opL_proper=> k y ?. by apply persistent_always.
  Qed.
...
Section gmap.
  ...
  Lemma big_sepM_always `{AffineBI PROP} Φ m :
  ...
  Proof.
    ...
    { by rewrite sep_elim_r. }
    rewrite !big_sepM_insert // bare_always_sep_dup.
    rewrite -assoc [(⬕ _ ∗ _)%I]comm -!assoc assoc. apply sep_mono.
    - rewrite (forall_elim i) (forall_elim x) pure_True ?lookup_insert //.
      by rewrite True_impl bare_always_elim wand_elim_l.
    - rewrite comm -IH /=.
      apply sep_mono_l, bare_mono, always_mono, forall_mono=> k.
...
Section gset.
  ...
  Lemma big_sepS_always `{AffineBI PROP} Φ X :
  ...
  Proof.
    ...
    { by rewrite sep_elim_r. }
    rewrite !big_sepS_insert // bare_always_sep_dup.
    rewrite -assoc [(⬕ _ ∗ _)%I]comm -!assoc assoc. apply sep_mono.
    - rewrite (forall_elim x) pure_True; last set_solver.
      by rewrite True_impl bare_always_elim wand_elim_l.
    - rewrite comm -IH /=. apply sep_mono_l, bare_mono, always_mono.
      apply forall_mono=> y. apply impl_intro_l, pure_elim_l=> ?.
      by rewrite pure_True ?True_impl; last set_solver.
  Qed.

  Global Instance big_sepS_empty_persistent `{AffineBI PROP} Φ :
    PersistentP ([∗ set] x ∈ ∅, Φ x).
